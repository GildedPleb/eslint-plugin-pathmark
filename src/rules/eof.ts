...
> = {
  create(context) {
    ...
    if (lastComment !== undefined) {
      const isLastThingInFile =
        text.trimEnd().endsWith(lastComment.value) ||
        text.trimEnd().endsWith(`${lastComment.value}*/`);
      if (isLastThingInFile) return {};
    }

    const insertionText = text.endsWith("\n") ? `// ${tag}\n` : `\n// ${tag}\n`;
    const range: readonly [number, number] = [text.length, text.length];
    ...
    for (const comment of eofComments) {
      ...
      context.report({
        ...
        fix(fixer) {
          const fixes = [];
          if (!hasAdditionalText) fixes.push(fixer.remove(comment));
  ...
  meta: {
    docs: {
      description:
        "enforce a specific tag to be present at the very end of a file.",
    },
    fixable: "code",
    messages: {
